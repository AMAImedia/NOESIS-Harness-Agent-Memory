...
class ControlPlaneUITests(unittest.TestCase):
    def test_ui_is_self_contained_read_only_surface(self):
        with HealthServer(port=0) as server:
            ...
            self.assertIn("/api/telemetry", body)
            self.assertIn("/api/telemetry/events", body)
            self.assertIn("/api/readiness", body)
            self.assertIn("/api/admin/migration-mode", body)
            self.assertIn("migration_readiness", body)
            self.assertIn("Operator-controlled migration", body)
            self.assertIn("Active child runtimes", body)
            self.assertIn("Learning review", body)
            self.assertIn("/api/learning/review", body)
            self.assertIn("provenance_status", body)
            self.assertIn("Automatic activation", body)
