...
class SecurityHoldoutTests(unittest.TestCase):
    ...
    def test_expected_rules_are_present_without_requiring_exact_rule_count(self):
        ...
        self.assertIn("shell_command_injection", results["inject-2"].rules)
        self.assertIn("path_traversal", results["path-1"].rules)
        self.assertIn("environment_secret_access", results["env-1"].rules)
