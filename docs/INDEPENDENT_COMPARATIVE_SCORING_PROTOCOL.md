...
A score becomes `score_available=true` only after all required lanes are readiness-passed, the complete case corpus is present for every lane, every signed case contains exactly the declared dimension field set with bounded numeric values in `[0,1]`, all receipts verify, and no mandatory safety failure is present. Missing, extra, nonnumeric or out-of-range dimensions invalidate the case; values are never imputed. The builder publishes deterministic per-lane dimension means and cross-lane means, but `score_claim` remains false until an independent review and the complete external evidence package authorize a comparative claim.
