...
- 2026-08-27: Gate 8 release review stages 1-4 and 9 pass under CPython 3.14.7 — runtime gate, 1110-test local suites (18 skipped), documentation audit, recall20 20/20 acc=1.00, workload20 score=0.8083, UTF-8 evidence regeneration and clean link/docs audits (status `passed_local`).
- 2026-08-27: the clean-tree release audit appendix is green (`release_audit.py` exit 0, parallel lanes 5/5, validator PASS) and the release review status doc is published with a Russian mirror, fixing the internal-release-candidate status and claim boundary (status `passed_local` / `local_documented`; public claim release stays gated on CA-issued signing, macOS parity, external lanes and human review).
...
- 2026-08-27: committed memory-quality evidence is regenerated as UTF-8 with the adversarial corpus v3 key included and byte-stable across runs (status `passed_local`; deterministic self-generated distributions, not an external benchmark).
...
- 2026-08-26: Phase A proxy-jail is implemented and wired into lane execution — a deny-by-default local forward proxy with exact-host allowlist, blind CONNECT tunneling without MITM and counted rejects; `pinned_runner_adapter.execute` fails closed without an explicit allowlist and an executed model_task lane ran through it cleanly (status `local_verified`; enforcement strength remains advisory per the sandbox design doc).
...
- 2026-08-26: first executed native Windows build evidence under CPython 3.14.7 / AMD64 — PyInstaller 6.22.2 produced `dist/noesis-harness.exe` with verifier-recorded shape, sha256 and honest signing status (development-unsigned; `signtool: not_run`, probe present rather than omitted). No macOS artifact or independent matching-host replay is claimed.
- 2026-08-26: the full signed operator evidence pipeline executed under CPython 3.14.7 as a byte-deterministic ten-file bundle with verified reproducibility receipt; clean-room replay regenerates it byte-identically and non-secret mirrors are committed, while release-readiness and post-transfer audit stay honestly `blocked` pending matching hosts and pinned lane receipts (status `passed_local`).
- 2026-08-27: the native Windows executable is signed with a development self-signed certificate (signtool 10.0.26100.0, root untrusted, sha256 `f17669d4…`) recorded in the native-artifact evidence (status `executed_unsigned_dev_signed`; not release-grade — a CA-issued certificate with a trusted root remains required).
...
- 2026-08-26: the three-lane `version_smoke` comparative baseline passed with all `task_success` accounted — opencode 1.0 (revision `b72b5000…`), deepseek_harness 1.0 (pinned sha `b150a551…`), hermes 1.0 after an executed repair recipe (4113 stdlib files restored from a clean uv tree plus venv reinstalls); the repaired hermes binary was subsequently removed by AV and re-quarantined, so its signed records stay evidence-valid while current runtime availability is lost (status `executed_passed_signed` / `executed_evidence_valid_runtime_unavailable`; smoke class only, single host, single runs).
- 2026-08-26: opencode `model_task` executed through the Phase A proxy jail under the full governance chain with a signed accepted record (`task_success` observed 1.0) — first model-backed lane execution; still one lane, one revision, no ranking.
- 2026-08-26: BYO operator-env credentials contract landed (`absent | operator_bypass_explicit:<NAMES>`): credential NAMES bind into the protocol fingerprint while secret values never enter specs, receipts or logs; blockers for each of the three lanes are recorded machine-readably in `docs/MODEL_TASK_3LANE_BLOCKERS.json` (status `local_verified`).
...
- 2026-08-26: first executed external lanes (`version_smoke` across three competitors, one opencode `model_task` run) and first native Windows build evidence under CPython 3.14.7 move preparation into execution, but they remain single-host single-run smoke/model probes with hermes runtime quarantined and no independent scoring or repeated runs; `comparative_ready` stays false and the bands deliberately remain **65–70%** local-OS and **25–35%** proven-claim until pinned repeated lanes and matching-host native parity exist.
- 2026-08-27: the Gate 8 release review executed on this host — stages 1-4 and 9 pass under CPython 3.14.7, the clean-tree release audit is green (5/5 parallel lanes + validator), the native executable is development-signed and the status doc is published with a Russian mirror; none of this opens a native parity or external lane, so the bands remain **65–70%** local-OS and **25–35%** proven-claim pending CA-issued signing, macOS parity, pinned repeated external lanes and human review.
