...
The schema `noesis.signed-release-readiness-receipt.v1` binds a release-readiness snapshot, a verified full-chain `release-gate.json`, and the validated test count used to produce the snapshot. It includes Python version, native/external status fields, deterministic `receipt_digest`, and an HMAC-SHA256 signature.
