...
| Operator administration | `AdministrativePolicyStore` is a reviewed, session-bound source for reviewer grant/revoke operations. It requires an active administrator session and `admin:reviewers` scope. `OperatorSessionActionExecutor` applies explicit, idempotent open/close actions; optional `POST /api/operator-sessions` and `POST /api/admin/reviewer-policy` only validate and delegate to injected handlers. |
...
Operator actions are separately replay-safe by `action_id`. A completed action returns its stored signed receipt instead of reapplying the state transition. Identity, session and optional scope mismatches fail closed and emit bounded `promotion_action_denied` telemetry; replay emits `promotion_action_replayed`. `OperatorSessionRegistry` reconstructs sessions from append-only events and denies inactive, closed or expired sessions. Administrative reviewer grants require an active administrator session with `admin:reviewers`; unauthorized grant/revoke attempts are rejected before policy mutation. Reviewer authorization is append-only and persistent: a grant must match operator/session and required scope, while missing or revoked grants deny. Approval requires an independent authorized reviewer relative to the experience owner. `reject` is allowed only from `review`; `rollback` is allowed only from `promoted` and does not activate anything. Promotion approval and activation remain outside task-event replay and are never inferred from a checkpoint.
