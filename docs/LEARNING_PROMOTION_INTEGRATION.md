...
| Operator approval UI | `PromotionApprovalAction` is a versioned non-secret action envelope for `approve`, `reject` and `rollback`. `OperatorAuthContext` must match operator identity and session and may enforce scopes. `ReviewerAuthorizationStore` persists reviewer grants and revocations; absent, revoked or scope-mismatched grants fail closed. `PromotionActionExecutor` applies only explicit proposal operations, requires an independent authorized reviewer, signs a receipt and stores an idempotent action record. Optional authenticated `POST /api/promotion-actions` validates the envelope and passes both action and configured operator context to an injected handler; the HealthServer never performs promotion itself. |
