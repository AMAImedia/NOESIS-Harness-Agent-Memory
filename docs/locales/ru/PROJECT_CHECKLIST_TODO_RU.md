...
| DONE-38 | Execution recovery Windows alias containment | `DONE / LOCAL VERIFIED` | Containment checks compare equivalent lexical/real path views for Windows 8.3 aliases without widening the root; targeted replay tests and compact regression smoke **84/84**, `ResourceWarning=0` |
| DONE-39 | Sandbox validation precedence | `DONE / LOCAL VERIFIED` | Invalid argv/workspace is rejected deterministically before unavailable-host check; Bubblewrap/child/skill focused suite **34/34**, `ResourceWarning=0`, unavailable Linux backend remains host-gated |
...
## 2026-08-21 — Roadmap checkpoint ancestry checkpoint
| ID | Область | Статус | Доказательство |
|---|---|---|---|
...
## 2026-08-22 — P11 terminal/web Python 3.14 verification

| ID | Область | Статус | Доказательство |
|---|---|---|---|
| P11-01-PY314 | Terminal/web control plane | `DONE / LOCAL VERIFIED` | `HealthServer`, durable session API, auth, UI contract, portable launcher, SSE stream и `terminal_client` прошли `34/34` focused tests на Python 3.14 при `ResourceWarning=0`; full suite `817/817`. |
| P11-02 | Native desktop shell | `NOT_RUN / ENVIRONMENT-GATED` | Native Tauri/Electron shell, native signing и native host parity не запускаются в Linux local lane. |

Machine-readable evidence: `docs/P11_TERMINAL_WEB_PY314_EVIDENCE.json`.

## 2026-08-22 — P12 Python 3.14 source packaging verification

| ID | Область | Статус | Доказательство |
|---|---|---|---|
| P12-01-PY314 | Source-portable artifact and interpreter probe | `DONE / LOCAL VERIFIED` | `verify_python314.py` passes on Python `3.14.7`; portable artifact verifier, native artifact evidence and launcher tests pass `14/14` with `ResourceWarning=0`. |
