...
` scripts/run_external_lane.py` связывает pinned spec с adapter и evidence pipeline в безопасном режиме. Без `--execute` создаётся только plan с `execution=not_started`; `--execute` без `--approve` возвращает `denied/not_run`. Только `--execute --approve` может начать процесс, после чего результат фиксируется как structured `started` outcome с status, return code, timeout и redacted output. Перед запуском durable journal обязан записать `started`; обычный возврат записывается как terminal `completed`, а timeout или исключение — как `abandoned`. Для timeout CLI возвращает nonzero, а recovery требует новый approval.

Для Hermes/OpenCode/DeepSeek Harness оператор сначала должен получить exact immutable revision, model/provider, task-manifest SHA-256, protocol fingerprint, required seed digest и disposable workspace, затем выполнить strict manifest validation, capability-aware dry-run и проверить plan. Нельзя подставлять floating revision, shell string, shared workspace, credentials или неподтверждённый executable. Approval использует `noesis.external-approval.v1`: HMAC receipt связывает exact command digest, argv-derived plan, workspace, revision, fingerprint и task-manifest digest, имеет bounded expiry и должен быть ровно один раз consumed через transactional SQLite/WAL receipt store. При concurrent consume побеждает только один процесс, committed consume сохраняется после reopen, а malformed/corrupted store fail-closed. Durable execution journal хранит states `consumed`, `started`, `completed`, `abandoned`; interrupted/abandoned execution не переиспользует receipt автоматически и требует новый approval, а completed получает `no_replay`. Отсутствующий, expired, mutated или replayed receipt остаётся denied. Публикация evidence выполняется отдельным ingestion/signing шагом.
