...
При включённом режиме `Leases(..., one_lease_per_holder=True)` один agent holder может иметь только один активный task lease. Вторая заявка на другой task fail-closed возвращает `reason: holder_lease_active`; повторная заявка на тот же task остаётся idempotent. `SafeParallelExecutor` запрещает shared workspace и недопустимые capabilities, а общие файлы, master checklist, финальная интеграция и commit принадлежат только coordinator.
