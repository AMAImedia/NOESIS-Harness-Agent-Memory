...
`score_available=true` разрешается только после readiness-pass всех required lanes, полного case corpus для каждой lane, exact declared dimension field set в каждом signed case, status каждой dimension равен `observed`, bounded numeric values в диапазоне `[0,1]`, успешной проверки всех receipts и отсутствия mandatory safety failure. Missing, extra, unobserved, nonnumeric или out-of-range dimensions инвалидируют case; значения никогда не impute-ятся.
 Builder публикует deterministic per-lane и cross-lane dimension means, но `score_claim` остаётся false до independent review и полного external evidence package.
