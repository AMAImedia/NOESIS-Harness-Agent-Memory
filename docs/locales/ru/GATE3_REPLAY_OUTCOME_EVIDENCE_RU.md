...
Completeness projection также сохраняется как signed `noesis.recovery-replay-evidence-completeness-snapshot.v1`, включая deterministic `completeness_digest` по ordered records и verified catalog digest. Startup-style replay audit может явно требовать этот durable snapshot; exact replay использует такой режим и поэтому не может пройти только на заново вычисленной in-memory projection при missing durable snapshot. Затем signed snapshot проверяется против current bundle и digest после commit manifest gate. Missing, corrupt, path-mismatched, tampered или drifted completeness snapshot fail-closed и никогда не создаётся молча заново во время replay.
