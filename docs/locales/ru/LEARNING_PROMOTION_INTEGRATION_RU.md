...
| Operator approval UI | `PromotionApprovalAction` — versioned non-secret envelope для `approve`, `reject`, `rollback`. `PromotionActionExecutor` выполняет только explicit proposal operations, требует independent reviewer, подписывает receipt и хранит idempotent action record. Optional `POST /api/promotion-actions` валидирует envelope и делегирует injected handler; HealthServer сам promotion не выполняет. |
...
Operator actions отдельно replay-safe по `action_id`: completed action возвращает сохранённый signed receipt и не повторяет transition. Approval требует independent reviewer относительно experience owner; `reject` допустим только из `review`, `rollback` — только из `promoted` и не активирует skill. Approval и activation никогда не выводятся из task-event checkpoint.
