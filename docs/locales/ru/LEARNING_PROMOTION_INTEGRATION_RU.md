...
| Operator approval UI | `PromotionApprovalAction` — versioned non-secret envelope для `approve`, `reject`, `rollback`. `OperatorAuthContext` обязан совпадать по operator identity и session и может проверять scopes. `ReviewerAuthorizationStore` persistent хранит grants/revocations; missing, revoked или scope-mismatched grant fail closed. `PromotionActionExecutor` выполняет только explicit proposal operations, требует independent authorized reviewer, подписывает receipt и хранит idempotent action record. Optional `POST /api/promotion-actions` валидирует envelope и передаёт action вместе с configured context в injected handler; HealthServer сам promotion не выполняет. |
