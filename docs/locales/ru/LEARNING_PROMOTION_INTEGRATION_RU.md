...
| Operator administration | `AdministrativePolicyStore` — reviewed session-bound source для grant/revoke reviewer operations. Требует active administrator session и `admin:reviewers`. `OperatorSessionActionExecutor` выполняет explicit idempotent open/close actions; optional `POST /api/operator-sessions` и `POST /api/admin/reviewer-policy` только валидируют и делегируют injected handlers. |
