...
The same snapshot may include `import_validation` with bounded status `blocked`, `accepted_not_run`, `accepted` or `unavailable`, plus at most 32 redacted drift/error reasons. The projection forcibly reports `score_claim=false` and `external_execution_claim=false`, even if an upstream provider returns conflicting values. This is informational telemetry only: the UI and SSE surface expose no import action, provider command, approval consumption or score mutation.
