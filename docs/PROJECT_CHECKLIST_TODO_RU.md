...
| P8-01 | Provider invocation adapters | Агент | `IMPLEMENTED / LOCAL VERIFIED / APPROVAL LAYER NEXT` | `provider_invocation.py`: explicit OpenAI-compatible DeepSeek/Hermes call, pinned model, capability gates, credential resolver only at invocation, bounded request/response, structured non-executing output; 3 focused tests; 211/211 full tests on local Python 3.12; Python 3.14 verification pending |
