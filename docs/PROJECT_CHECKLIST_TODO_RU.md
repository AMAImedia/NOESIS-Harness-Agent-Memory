...
| P9-01 | Isolated child execution runtime | Агент | `IMPLEMENTED / LOCAL VERIFIED / HARDENED SANDBOX NEXT` | `child_execution.py`: explicit Gatekeeper commit, argv allowlist, shell=false, workspace containment, symlink/traversal rejection, environment allowlist, timeout/output budgets, process termination/recovery; 5 focused tests; 221/221 full tests on local Python 3.12. This is a process boundary, not a claimed OS sandbox; network is fail-closed. |
...
| P10-01 | Diff/patch review and per-agent workspaces | Агент | `IMPLEMENTED / LOCAL VERIFIED / MERGE GATE NEXT` | `workspaces.py`: isolated per-agent roots, safe paths, immutable marker, SHA-256 snapshots, added/modified/deleted patch proposal and approved/rejected review; 4 focused tests; 225/225 full tests on local Python 3.12; Python 3.14 verification pending. Merge/publish remains explicit Gatekeeper action. |
| P10-02 | Session resume and real multi-agent execution | Агент | `IMPLEMENTED / LOCAL VERIFIED / PROVIDER EXECUTION NEXT` | `multi_agent_runtime.py`: registered agent identities, exclusive task claims, per-agent workspace assignment, review completion, handoff and resume view; `experience_reuse.py`: provenance-required bounded reuse, scope/sensitivity denial, deterministic success/recency ordering and explainable char/item budgets; 4 focused tests. Parallel provider execution and cancellation budgets remain next. |
...
| P13-01 | Comparative A/B and task benchmarks | Агент | `CONTRACT PROTOCOL + LOCAL BASELINE READY / EXTERNAL A-B NOT RUN` | `docs/COMPETITIVE_BENCHMARK_PROTOCOL_RU.md` defines fixed task suite, metrics, stop conditions and reporting schema. `scripts/benchmark_contracts.py` ran 10/10 local contract cases passed on Python 3.12. External Hermes/OpenCode/DeepSeek runs and native Windows/macOS measurements remain explicitly `not_run`. |
...
| DOC-02 | Local relative-link audit | `DONE / LOCAL VERIFIED` | 70 Markdown files, 31 local links, missing targets `0`; generated runtime docs исключены |
...
| DOC-05 | Parallel documentation evidence | `DONE / LOCAL VERIFIED` | 4/4 lanes passed; 4 unique workspaces; no network/credentials/model code; file `docs/PARALLEL_DOCUMENTATION_EVIDENCE.json`; SHA-256 `312c7530c14d4854d65b40d735bd1bf782a254aafdbc37404a51a12677a36470` |
