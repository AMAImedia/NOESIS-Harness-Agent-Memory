...
This command is operator-owned, not autonomous. No background process polls it, telemetry cannot invoke it, and the endpoint does not manufacture approval IDs. The caller must supply a fresh approval produced by the delegated resume lifecycle. Existing Actions claim, workspace binding, sandbox, child-runtime and execution-receipt guards remain mandatory.
