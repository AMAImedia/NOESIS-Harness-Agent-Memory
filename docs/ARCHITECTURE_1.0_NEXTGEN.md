...
| Parallel resilience and evidence | `SafeParallelExecutor`, `CrossAgentLeakageSuite`, `WorkProductBenchmarkEvaluator` | Explicit retry limit with action reclaim, cancellation non-retry, 12 deterministic cross-agent leakage holdouts and separate correctness/delivery/leakage/recovery/reviewer-time metrics |
| Workload benchmark | `MultiAgentWorkloadRunner`, `DurableWorkloadAggregator` | Multiple parallel deterministic lanes, crash injection before write/after write/after read, bounded retry, SQLite/WAL result aggregation, duplicate replay, content-conflict rejection and repeated percentile report |
| Active delegation leakage | `ActiveDelegationLeakageSuite` | Concurrent sibling-read/write, absolute-path and traversal probes remain denied while multiple lanes are active |
| Memory quality evidence | `MemoryQualityEvaluator` | Separate recall, attribution precision, conflict resolution, temporal order, compaction retention, hard budget and leakage-free metrics; no hidden model grading |
...
The suites cover tamper detection, Windows SQLite cleanup, path denial, private-scope isolation, idempotency, compaction retention, gate approval, DAG cycles, atomic vault writes, staged skills, child-runtime manifest/grant enforcement, HMAC receipt persistence/tamper rejection, durable patch review, interrupted-run recovery, timeout/process-tree recovery, credential output blocking, adversarial Bubblewrap filesystem/network isolation, typed multi-agent work products, independent review, stale-base denial, explicit commit and session resume/replay, bounded retry/reclaim, cancellation non-retry, 12-case cross-agent leakage holdouts, deterministic work-product quality metrics, parallel workload crash injection at three points, active-lane workspace escape denial, durable aggregation replay/conflict rejection, repeated-run percentile reporting, four simultaneous active-delegation leakage probes and memory quality metrics for recall, attribution, conflict, temporal order, compaction retention, hard budget and leakage-free behavior. Native Windows/macOS sandbox evidence remains `not_run` until matching hosts are available.
