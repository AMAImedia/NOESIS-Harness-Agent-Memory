...
The constructor rejects non-positive or non-integer `max_turns` and rejects a non-callable injected clock before any lease can be acquired. Action and judge outputs must be mappings; malformed outputs are bounded as failures. Telemetry append failures are isolated and do not convert a valid control result into an execution failure.
