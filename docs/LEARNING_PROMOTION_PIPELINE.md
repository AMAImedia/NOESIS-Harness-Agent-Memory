...
A proposal is review-only until explicit approval. Each proposal carries a `provenance_digest` binding its receipt facts, evaluation record, skill name and content digest. Approval and promotion recompute that digest and reject stale or tampered records. Approval requires an operator identity and a passing caller-supplied test hook. Promotion rejects content digest mismatch, missing approval, failed verification, duplicate version directory creation, and any exception raised by the verification callback. The module writes no executable entrypoint and never invokes skill content.

The operator review surface is exposed through bounded `noesis.learning-review-snapshot.v1` metadata and `/api/learning/review`; it returns proposal/evaluator identifiers, digests, states and provenance status, but never skill or payload content. The endpoint is authenticated by the server boundary, read-only, and fails closed when no provider is configured or the provider errors. `automatic_activation` is always false in review metadata. Activation is represented only by an `ACTIVE` pointer to an immutable version and is not performed by capture, evaluation, proposal, review snapshot, or evaluator registration. Rollback removes that pointer or restores the previous pointer. A promotion receipt is HMAC-SHA256 signed over the proposal ID, skill name, immutable version, and activation flag. Signature verification uses constant-time comparison.
