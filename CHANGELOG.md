...
- Phase B AppContainer probes (Gate 26): additive `profile_sid_probe()`, `capability_inventory()`, and `run_probe()` in `noesis_harness/appcontainer_backend.py` — ctypes introspection only, never create a profile, never spawn a child; 12 new tests (27 total in the module). Host evidence recorded in `docs/MODEL_TASK_SANDBOX_DESIGN.md` §5: all three `kernel32` exports callable, `GetAppContainerSid` absent on this userenv.dll, SID derivation succeeds via `DeriveAppContainerSidFromAppContainerName`; execution stays unbound because profile creation requires elevation and token derivation needs the profile to exist.
