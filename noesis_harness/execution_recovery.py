...
class ExecutionRecoveryExecutor:
    ...
    def _replay_inventory_verification_readiness_path(self) -> str:
        return str(self.events.path) + ".replay-inventory-verification-readiness.json"
    ...
    def _replay_manifest_binding_readiness_path(self) -> str:
        return str(self.events.path) + ".replay-manifest-readiness-binding-chain-readiness.json"
    ...
    def repair_replay_evidence_finalization(self) -> Mapping[str, Any]:
        ...
        verification_snapshot = self._persist_inventory_verification_receipt()
        return {"status": "passed", "repaired": True, "archived_partial_finalization": archive_path, "finalization": self.verify_replay_evidence_finalization(), "repair_receipt": repair_snapshot, "repair_chain_readiness": readiness_snapshot, "finalized_inventory": inventory_snapshot, "inventory_verification": verification_snapshot}
    ...
    def _persist_inventory_verification_receipt(self) -> Mapping[str, Any]:
        inventory = self.verify_finalized_evidence_inventory()["payload"]
        readiness = self.audit_replay_chain_readiness()
        payload = {"schema_version": "noesis.recovery-replay-inventory-verification.v1", "status": "passed", "event_path": str(self.events.path), "inventory_path": self._replay_finalized_inventory_path(), "inventory_digest": inventory["inventory_digest"], "generation_id": inventory["generation_id"], "generation_digest": inventory["generation_digest"], "chain_root_digest": inventory["chain_root_digest"], "readiness_status": readiness["status"], "finalization_status": "finalized", "verification_path": self._replay_inventory_verification_path()}
        payload["verification_digest"] = request_fingerprint({key: value for key, value in payload.items() if key != "verification_digest"})
        snapshot = {"payload": payload, "signature": _snapshot_signature(payload, self.receipt_store.signing_key)}
        ...
        self._persist_inventory_verification_chain_readiness_snapshot()
        return snapshot

    def _persist_inventory_verification_chain_readiness_snapshot(self) -> Mapping[str, Any]:
        chain = self.verify_inventory_verification_chain()
        inventory = self.verify_finalized_evidence_inventory()["payload"]
        receipt = self.verify_inventory_verification_receipt()["payload"]
        payload = {"schema_version": "noesis.recovery-replay-inventory-verification-chain-readiness.v1", "status": "passed", "event_path": str(self.events.path), "chain_path": self._replay_inventory_verification_chain_path(), "inventory_path": self._replay_finalized_inventory_path(), "inventory_digest": inventory["inventory_digest"], "verification_digest": receipt["verification_digest"], "tip_digest": chain["tip_digest"], "run_count": chain["run_count"], "readiness_result": "passed", "verification_result": "passed", "readiness_path": self._replay_inventory_verification_readiness_path()}
    ...
    def _bind_commit_manifests_to_verification_readiness(self, readiness_snapshot: Mapping[str, Any]) -> None:
        ...
        self._persist_manifest_binding_receipt(readiness_snapshot)

    def _persist_manifest_binding_receipt(self, readiness_snapshot: Mapping[str, Any]) -> Mapping[str, Any]:
        readiness = readiness_snapshot["payload"]
        inventory = self.verify_finalized_evidence_inventory()["payload"]
        parent = os.path.dirname(os.path.abspath(str(self.events.path))) or "."
        prefix = os.path.basename(str(self.events.path)) + ".replay-commit."
        manifests = []
        for name in sorted(os.listdir(parent)):
            if name.startswith(prefix) and name.endswith(".json"):
                path = os.path.join(parent, name)
                manifests.append({"path": path, "sha256": self._sha256_file(path)})
        payload = {"schema_version": "noesis.recovery-replay-manifest-readiness-binding.v1", "status": "passed", "event_path": str(self.events.path), "readiness_path": self._replay_inventory_verification_readiness_path(), "readiness_digest": request_fingerprint(readiness), "inventory_digest": inventory["inventory_digest"], "chain_root_digest": inventory["chain_root_digest"], "verification_chain_tip_digest": readiness["tip_digest"], "manifests": manifests, "receipt_path": self._replay_manifest_binding_receipt_path()}
        payload["binding_digest"] = request_fingerprint({key: value for key, value in payload.items() if key != "binding_digest"})
        snapshot = {"payload": payload, "signature": _snapshot_signature(payload, self.receipt_store.signing_key)}
    ...
    def _append_manifest_binding_run(self, receipt_snapshot: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        with open(path, "a", encoding="utf-8") as handle:
            ...
            os.fsync(handle.fileno())
        self._make_readonly((path,))
        self._persist_manifest_binding_chain_readiness_snapshot()
        return run

    def _persist_manifest_binding_chain_readiness_snapshot(self) -> Mapping[str, Any]:
        chain = self.verify_manifest_binding_chain()
        receipt = self.verify_manifest_binding_receipt()["payload"]
        payload = {"schema_version": "noesis.recovery-replay-manifest-readiness-binding-chain-readiness.v1", "status": "passed", "event_path": str(self.events.path), "chain_path": self._replay_manifest_binding_chain_path(), "receipt_path": self._replay_manifest_binding_receipt_path(), "binding_digest": receipt["binding_digest"], "inventory_digest": receipt["inventory_digest"], "chain_root_digest": receipt["chain_root_digest"], "tip_digest": chain["tip_digest"], "run_count": chain["run_count"], "readiness_result": "passed", "verification_result": "passed", "readiness_path": self._replay_manifest_binding_readiness_path()}
        payload["readiness_digest"] = request_fingerprint({k:v for k,v in payload.items() if k != "readiness_digest"})
        snapshot = {"payload": payload, "signature": _snapshot_signature(payload, self.receipt_store.signing_key)}
        _atomic_write_json(self._replay_manifest_binding_readiness_path(), snapshot)
        self._make_readonly((self._replay_manifest_binding_chain_path(), self._replay_manifest_binding_readiness_path()))
        return snapshot
    def verify_manifest_binding_chain_readiness_snapshot(self) -> Mapping[str, Any]:
        try:
            with open(self._replay_manifest_binding_readiness_path(), "r", encoding="utf-8") as handle:
                snapshot=json.load(handle, object_pairs_hook=_reject_duplicate_json_keys)
            payload=snapshot["payload"]; signature=str(snapshot["signature"])
        except (_DuplicateJSONKeyError,OSError,KeyError,TypeError,ValueError,json.JSONDecodeError) as exc:
            raise ExecutionRecoveryError("recovery_manifest_binding_readiness_corrupt") from exc
        if not isinstance(payload, Mapping) or not hmac.compare_digest(signature, _snapshot_signature(payload, self.receipt_store.signing_key)):
            raise ExecutionRecoveryError("recovery_manifest_binding_readiness_signature_invalid")
        chain=self.verify_manifest_binding_chain(); receipt=self.verify_manifest_binding_receipt()["payload"]
        expected={"schema_version":"noesis.recovery-replay-manifest-readiness-binding-chain-readiness.v1","status":"passed","event_path":str(self.events.path),"chain_path":self._replay_manifest_binding_chain_path(),"receipt_path":self._replay_manifest_binding_receipt_path(),"binding_digest":receipt["binding_digest"],"inventory_digest":receipt["inventory_digest"],"chain_root_digest":receipt["chain_root_digest"],"tip_digest":chain["tip_digest"],"run_count":chain["run_count"],"readiness_result":"passed","verification_result":"passed","readiness_path":self._replay_manifest_binding_readiness_path()}
        expected["readiness_digest"]=request_fingerprint({k:v for k,v in expected.items() if k != "readiness_digest"})
        if set(payload)!=set(expected) or any(payload.get(k)!=v for k,v in expected.items()) or not self._is_readonly(self._replay_manifest_binding_readiness_path()):
            raise ExecutionRecoveryError("recovery_manifest_binding_readiness_drift")
        return {"status":"passed","payload":dict(payload),"signature":signature}
    ...
    def verify_manifest_binding_receipt(self) -> Mapping[str, Any]:
        try:
            with open(self._replay_manifest_binding_receipt_path(), "r", encoding="utf-8") as handle:
                snapshot = json.load(handle, object_pairs_hook=_reject_duplicate_json_keys)
            payload = snapshot["payload"]
            signature = str(snapshot["signature"])
        except (_DuplicateJSONKeyError, OSError, KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
            raise ExecutionRecoveryError("recovery_manifest_binding_receipt_corrupt") from exc
        if not isinstance(payload, Mapping) or not hmac.compare_digest(signature, _snapshot_signature(payload, self.receipt_store.signing_key)):
            raise ExecutionRecoveryError("recovery_manifest_binding_receipt_signature_invalid")
        readiness = self.verify_inventory_verification_chain_readiness_snapshot()["payload"]
        inventory = self.verify_finalized_evidence_inventory()["payload"]
        manifests = [{"path": path, "sha256": self._sha256_file(path)} for path in sorted(record["path"] for record in payload.get("manifests", ()))]
        expected = {"schema_version": "noesis.recovery-replay-manifest-readiness-binding.v1", "status": "passed", "event_path": str(self.events.path), "readiness_path": self._replay_inventory_verification_readiness_path(), "readiness_digest": request_fingerprint(readiness), "inventory_digest": inventory["inventory_digest"], "chain_root_digest": inventory["chain_root_digest"], "verification_chain_tip_digest": readiness["tip_digest"], "manifests": manifests, "receipt_path": self._replay_manifest_binding_receipt_path()}
        expected["binding_digest"] = request_fingerprint({key: value for key, value in expected.items() if key != "binding_digest"})
        if set(payload) != set(expected) or any(payload.get(key) != value for key, value in expected.items()) or not self._is_readonly(self._replay_manifest_binding_receipt_path()):
            raise ExecutionRecoveryError("recovery_manifest_binding_receipt_drift")
        return {"status": "passed", "payload": dict(payload), "signature": signature}

    def verify_inventory_verification_chain_readiness_snapshot(self) -> Mapping[str, Any]:
        try:
            with open(self._replay_inventory_verification_readiness_path(), "r", encoding="utf-8") as handle:
                snapshot = json.load(handle, object_pairs_hook=_reject_duplicate_json_keys)
            payload = snapshot["payload"]
            signature = str(snapshot["signature"])
        except (_DuplicateJSONKeyError, OSError, KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
            raise ExecutionRecoveryError("recovery_inventory_verification_readiness_corrupt") from exc
        if not isinstance(payload, Mapping) or not hmac.compare_digest(signature, _snapshot_signature(payload, self.receipt_store.signing_key)):
            raise ExecutionRecoveryError("recovery_inventory_verification_readiness_signature_invalid")
        chain = self.verify_inventory_verification_chain()
        inventory = self.verify_finalized_evidence_inventory()["payload"]
        receipt = self.verify_inventory_verification_receipt()["payload"]
        expected = {"schema_version": "noesis.recovery-replay-inventory-verification-chain-readiness.v1", "status": "passed", "event_path": str(self.events.path), "chain_path": self._replay_inventory_verification_chain_path(), "inventory_path": self._replay_finalized_inventory_path(), "inventory_digest": inventory["inventory_digest"], "verification_digest": receipt["verification_digest"], "tip_digest": chain["tip_digest"], "run_count": chain["run_count"], "readiness_result": "passed", "verification_result": "passed", "readiness_path": self._replay_inventory_verification_readiness_path()}
        expected["readiness_digest"] = request_fingerprint({key: value for key, value in expected.items() if key != "readiness_digest"})
        if set(payload) != set(expected) or any(payload.get(key) != value for key, value in expected.items()) or not self._is_readonly(self._replay_inventory_verification_readiness_path()):
            raise ExecutionRecoveryError("recovery_inventory_verification_readiness_drift")
        return {"status": "passed", "payload": dict(payload), "signature": signature}
    ...
    def verify_inventory_verification_receipt(self) -> Mapping[str, Any]:
        try:
            with open(self._replay_inventory_verification_path(), "r", encoding="utf-8") as handle:
                snapshot = json.load(handle, object_pairs_hook=_reject_duplicate_json_keys)
            payload = snapshot["payload"]
            signature = str(snapshot["signature"])
        except (_DuplicateJSONKeyError, OSError, KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
            raise ExecutionRecoveryError("recovery_inventory_verification_corrupt") from exc
        if not isinstance(payload, Mapping) or not hmac.compare_digest(signature, _snapshot_signature(payload, self.receipt_store.signing_key)):
            raise ExecutionRecoveryError("recovery_inventory_verification_signature_invalid")
        inventory = self.verify_finalized_evidence_inventory()["payload"]
        readiness = self.audit_replay_chain_readiness()
        expected = {"schema_version": "noesis.recovery-replay-inventory-verification.v1", "status": "passed", "event_path": str(self.events.path), "inventory_path": self._replay_finalized_inventory_path(), "inventory_digest": inventory["inventory_digest"], "generation_id": inventory["generation_id"], "generation_digest": inventory["generation_digest"], "chain_root_digest": inventory["chain_root_digest"], "readiness_status": readiness["status"], "finalization_status": "finalized", "verification_path": self._replay_inventory_verification_path()}
        expected["verification_digest"] = request_fingerprint(expected)
        if set(payload) != set(expected) or any(payload.get(key) != value for key, value in expected.items()) or not self._is_readonly(self._replay_inventory_verification_path()):
            raise ExecutionRecoveryError("recovery_inventory_verification_drift")
        return {"status": "passed", "payload": dict(payload), "signature": signature}
    ...
    def audit_replay_chain_readiness(self) -> Mapping[str, Any]:
        ...
        return result
    def audit_inventory_verification_chain_readiness(self) -> Mapping[str, Any]:
        path = self._replay_inventory_verification_chain_path()
        if not os.path.exists(path):
            return {"schema_version": "noesis.recovery-replay-inventory-verification-chain-readiness.v1", "status": "missing", "path": path, "run_count": 0, "readonly": False}
        try:
            entries = self._read_inventory_verification_chain()
        except ExecutionRecoveryError as exc:
            reason = str(exc)
            if reason == "recovery_inventory_verification_chain_order_invalid":
                status = "reordered"
            elif reason == "recovery_inventory_verification_chain_partial_record":
                status = "partial"
            elif reason == "recovery_inventory_verification_chain_file_identity":
                status = "corrupt"
            else:
                status = "corrupt"
            return {"schema_version": "noesis.recovery-replay-inventory-verification-chain-readiness.v1", "status": status, "path": path, "run_count": 0, "readonly": self._is_readonly(path), "error": reason}
        readonly = self._is_readonly(path)
        status = "passed" if entries and readonly else "partial"
        try:
            receipt = self.verify_inventory_verification_receipt()["payload"]
            if entries and (entries[-1].get("inventory_digest") != receipt.get("inventory_digest") or entries[-1].get("verification_digest") != receipt.get("verification_digest")):
                raise ExecutionRecoveryError("recovery_inventory_verification_chain_drift")
        except ExecutionRecoveryError as exc:
            status = "corrupt"
            return {"schema_version": "noesis.recovery-replay-inventory-verification-chain-readiness.v1", "status": status, "path": path, "run_count": len(entries), "readonly": readonly, "error": str(exc), "tip_digest": str(entries[-1].get("verification_run_digest", "")) if entries else ""}
        return {"schema_version": "noesis.recovery-replay-inventory-verification-chain-readiness.v1", "status": status, "path": path, "run_count": len(entries), "readonly": readonly, "run_ids": [int(entry["run_id"]) for entry in entries], "tip_digest": str(entries[-1]["verification_run_digest"]) if entries else ""}
    ...
    def verify_replay_evidence_readiness(self, *, require_finalized: bool = False) -> Mapping[str, Any]:
        ...
        if repair_chain.get("status") in {"corrupt", "partial", "reordered"} and repair_chain.get("error"):
            raise ExecutionRecoveryError(str(repair_chain["error"]))
        verification_chain_readiness = self.audit_inventory_verification_chain_readiness()
        if verification_chain_readiness.get("status") == "corrupt" and os.path.exists(self._replay_inventory_verification_chain_path()):
            raise ExecutionRecoveryError(str(verification_chain_readiness.get("error", "recovery_inventory_verification_chain_drift")))
        ...
        verification_chain_readiness_snapshot = None
        ...
        if os.path.exists(self._replay_inventory_verification_readiness_path()):
            verification_chain_readiness_snapshot = self.verify_inventory_verification_chain_readiness_snapshot()
        ...
        manifest_binding_readiness = None
        if os.path.exists(self._replay_manifest_binding_readiness_path()):
            manifest_binding_readiness = self.verify_manifest_binding_chain_readiness_snapshot()
